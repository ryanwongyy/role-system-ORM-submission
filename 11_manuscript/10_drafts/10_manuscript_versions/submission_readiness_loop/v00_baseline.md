...
Before describing the audit, we clarify the rubric's paradigm location. The audit operates within the modernist measurement tradition of organizational research, in the construct-validity lineage. The rubric is constructed under both *deductive* and *inductive* forces: the four allocation states (robust → conditional → transformed → shrinking) are deductively anchored to the automation-augmentation paradox literature (Raisch & Krakowski, 2021; Autor, 2015) on how human work is reorganized as machines are embedded in the workflow; the two specification operations (split pressure / merge pressure) emerged inductively through constant comparison across cases that the allocation continuum could not accommodate. The audit further engages the configurational-theorizing, content-analysis, typology-as-theory, organizational-routines (ostensive aspect), and high-reliability-organizing traditions. Adjacent traditions — paradox-theoretic, institutional, practice-theoretic, evolutionary — name rival readings the v1.0 rubric does not adjudicate; §6 identifies them.
...
The rubric in Table 1 was developed under both deductive and inductive forces. Behavioral anchors are operationalizations of the role-construct in the validity sense developed in measurement theory: anchors are observable features that license inferences about the underlying status, and the rubric's claim is to construct validity in the Cronbach-Meehl-Messick lineage (Cronbach & Meehl, 1955; Messick, 1995; Borsboom et al., 2004), where validity is a property of inferences rather than of the instrument alone.

*Deductive anchor (allocation continuum).* The four allocation states — robust → conditional → transformed → shrinking — are deductively anchored to the *automation-augmentation paradox* literature on how human work is reorganized when machines are embedded in the workflow (Raisch & Krakowski, 2021; Autor, 2015). *Robust* corresponds to the human-as-sole-performer pole (the augmentation-only end where AI substitution would degrade validity); *conditional* to moderator-gated mixed performance (the contingent-augmentation middle); *transformed* to content-redefinition with humans retained at the redefined task (the augmentation-with-content-shift register); and *shrinking* to substitution with humans as exception-handlers (the automation-leaning pole). The four states are not a priori partitions of the dimension; they are the discrete points along the augmentation–automation continuum at which case-text language consistently changed.

*Inductive emergence (specification operations).* The two specification operations — *split pressure* and *merge pressure* — emerged from constant comparison across cases that the deductive continuum could not accommodate. v0.2 added *split pressure* after Case 3 (Gligorić) showed *construct and perspective boundary setter* conflated diagnostic and sampling sub-activities; v0.3 added *merge pressure* after Case 10 (Hajishirzi) showed provenance and accountability inseparable. These operations revise the unit of analysis itself rather than the human-AI allocation within a fixed unit, and the typology-as-theory-building tradition treats this revisability as a productive feature (Doty & Glick, 1994; Fiss, 2011; Delbridge & Fiss, 2013; Cornelissen, 2017; Cornelissen et al., 2021). Furnari et al.'s (2021) configurational-theorizing heuristics — *scoping* and *linking* — operationalize split and merge pressure respectively at the role-cell level.

*Operational pragmatics.* Six is the smallest scheme that lets the four allocation states be coded separately (collapsing them loses the deductively anchored continuum) and lets segmentation and consolidation be coded as distinct specification operations (collapsing them loses the inductively emergent finding). Adding a seventh status (e.g., role *creation* or *destruction*) would either duplicate the allocation states or push reliability further down; the existing six already register substantial per-status κ disagreement on transition-coded categories (§6.2).

*Version history.* v0.1 carried the four allocation states; v0.2 added *split pressure*; v0.3 added *merge pressure*; v0.4 tightened the conditional-vs-transformed tie-breaker after Appendix B's reliability pass. Split pressure is recorded when (a) at least two cases within the same primary AI capability show status divergence across conflated sub-activities and (b) the divergence reflects distinct sub-activities rather than magnitude variation; *corpus pluralist* fails (b) and is excluded. Merge pressure is recorded when evidence for two roles is inseparable within a single case. A date-stamped codebook version history is released with the database. The four-case pilot also served as coder-development: the first coder re-coded Cases 1 and 2 under v0.4 to verify earlier codings remained stable; full-corpus stability is a v1.2 commitment.
...
The two specification operations — split pressure and merge pressure — emerged inductively from the audit (§2.3) when the deductively anchored allocation continuum could not accommodate the observed pattern. They operationalize Furnari et al.'s (2021) configurational-theorizing heuristics at the role-cell level: split pressure operates as the *scoping* heuristic (re-determining which sub-activities belong inside a role-cell and which belong outside), and merge pressure operates as *linking* (recombining two formerly distinct role-cells into a single configurational element). This section reports the cases that forced each operation. Some of the most important lessons in the repository are not about robustness or shrinkage. They are about mis-specified roles. The case on multi-perspective annotation shows why. Where disagreement across subgroups is itself substantively meaningful, a single correction target or majority-vote label can erase the quantity of interest. The paper therefore treats the distribution of perspectives as the inferential object rather than as annotation noise (Gligorić, n.d.). That is difficult to fit inside a single undifferentiated "construct and perspective boundary setter." The repository is right to register split pressure here — in jurisdictional terms, the diagnostic sub-activity (which categories exist) is segmenting from the sampling sub-activity (whose perspectives count), which were formerly welded together.
...
The six statuses sort onto two aggregate dimensions of distinct theoretical provenance (§2.3). The *allocation* axis (robust → conditional → transformed → shrinking) is deductively anchored to the automation-augmentation paradox (Raisch & Krakowski, 2021; Autor, 2015) and tracks how human performance yields scope, content, then volume. The *specification* axis (split and merge pressure) emerged inductively from constant comparison across cases (Gioia et al., 2013) and revises the unit of analysis itself. The two-axis structure was not visible at v0.1; it surfaced after Case 10's merge-pressure coding made it impossible to read split and merge as further points on the allocation axis. The typology is six rather than four because reconfiguration and reallocation are independent forms of jurisdictional revision (Abbott, 1988; Freidson, 2001).
...
The contribution is the comparative typology audit itself — the worked application of a six-status rubric to 36 source-backed cases of AI in research, anchored deductively to the automation-augmentation paradox and inductively to the specification operations the cases forced. The audit yields three released artifacts that operationalize the framework for downstream use: (1) the v1.0 rubric — a behaviorally-anchored instrument for coding role-status change at the (role × case) cell level; (2) the applied-use pipeline — case CSV, codebook, decision tree (Appendix E), reviewer-audit checklist (Appendix F) — at the v1.0.0-ORM-submission release tag; (3) the disconfirmation apparatus — pre-committed 2027 hold-out, human second-coder pass, and parallel-rubric application by independent groups on a 2027–2030 sample. The audit sits within computational social science (Lazer et al., 2009, 2020) at a level the existing CSS literature has under-theorized: not the pipeline (Hickman et al., 2022) and not the model, but the activity-level role within which both pipeline and model are embedded. We acknowledge a governmentality dimension: once the six statuses circulate as a measurement convention, researchers will plausibly organize their work around the rubric, which will partly *produce* the role-system it claims to describe; this is the inescapable side of any methodological-rubric output. We hope this audit serves as a catalyst for instrument-grade comparative audits of role-system change under AI embedding. The empirical terrain has changed: not from human research to machine research, but from one research role system to another.
